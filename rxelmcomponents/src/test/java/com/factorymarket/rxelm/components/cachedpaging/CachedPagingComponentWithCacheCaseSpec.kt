...
class CachedPagingComponentWithCacheCaseSpec : DescribeSpec({

    describe("second iteration of cacheable paging with cache") {

        val pagingComponent: CachedPagingComponent<String, Unit> =
            CachedPagingComponent(object : CachedPagingCommandsHandler<String, Unit> {
            ...
            context("user scrolled to the bottom of list") {
                ...
                context("two pages from cache arrived") {
                    it("should show two pages of items, show loader in the bottom, and allow new cache request ") {
                        val twoPageList = getItems(size = 2 * CACHE_PAGING_PAGE_SIZE)
